...
export class TraderUtils {
	...
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	public addAnastasiaToDb(traderDetailsToAdd: any, assort): void {
		this.ref.tables.traders[traderDetailsToAdd._id] = {
			assort: this.ref.jsonUtil.deserialize(
				this.ref.jsonUtil.serialize(assort),
			) as ITraderAssort,
			base: this.ref.jsonUtil.deserialize(
				this.ref.jsonUtil.serialize(traderDetailsToAdd),
			) as ITraderBase,
			questassort: {
				started: {},
				success: {},
	...
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	public addEvelynToDb(traderDetailsToAdd: any, assort): void {
		this.ref.tables.traders[traderDetailsToAdd._id] = {
			assort: this.ref.jsonUtil.deserialize(
				this.ref.jsonUtil.serialize(assort),
			) as ITraderAssort,
	...
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	public addSvetlanaToDb(traderDetailsToAdd: any, assort): void {
		this.ref.tables.traders[traderDetailsToAdd._id] = {
			assort: this.ref.jsonUtil.deserialize(
				this.ref.jsonUtil.serialize(assort),
			) as ITraderAssort,
	...
	public addTraderToLocales(
		...
		const locales = Object.values(this.ref.tables.locales.global) as Record<
			string,
			string
		>[];
